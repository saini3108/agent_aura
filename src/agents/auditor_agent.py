...
class AuditorAgent:
    ...
    def run(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            # Handle both direct agent outputs and workflow context
            previous_outputs = context.get('previous_outputs', {})
            if not previous_outputs:
                # Try to get outputs from direct context keys
                previous_outputs = {
                    'step_0': context.get('analyst_output', {}),
                    'step_1': context.get('validator_output', {}),
                    'step_2': context.get('documentation_output', {}),
                    'step_4': context.get('reviewer_output', {})
                }
            ...
            audit_results['executive_summary'] = executive_summary
            ...
            key_metrics = self._extract_key_metrics(validator_results, audit_findings, compliance_assessment)
            audit_results['metrics'] = key_metrics

            # Generate actionable recommendations
            recommendations = self._generate_actionable_recommendations(audit_findings, compliance_assessment)
            ...
            return {
                ...
                'status': 'error',
                'error': str(e),
                'executive_summary': f'Audit failed due to error: {str(e)}',
                'metrics': {},
                'recommendations': ['Address execution error before proceeding']
            }
    ...
    def _generate_executive_summary(self, analyst_results: Dict[str, Any], 
                                   validator_results: Dict[str, Any],
                                   documentation_results: Dict[str, Any],
                                   reviewer_results: Dict[str, Any]) -> str:
        """Generate comprehensive executive summary"""

        summary_parts = []

        # Overall status assessment
        completed_agents = sum(1 for result in [analyst_results, validator_results, documentation_results, reviewer_results] 
                              if result.get('status') == 'completed')

        summary_parts.append(f"Comprehensive audit completed with {completed_agents}/4 agent validations successful.")
        ...
        if validator_results.get('status') == 'completed':
            ...
            auc = metrics.get('auc', 0.0)
            if auc >= 0.8:
                summary_parts.append(f"Model demonstrates excellent discriminatory power (AUC: {auc:.3f}).")
            elif auc >= 0.7:
                summary_parts.append(f"Model shows good performance (AUC: {auc:.3f}).")
            elif auc >= 0.6:
                summary_parts.append(f"Model meets minimum regulatory standards (AUC: {auc:.3f}).")
            else:
                summary_parts.append(f"Model performance below regulatory threshold (AUC: {auc:.3f}).")
        ...
        if analyst_results.get('status') == 'completed':
            ...
            if total_missing == 0:
                summary_parts.append("Data quality assessment shows excellent completeness.")
            elif total_missing < 100:
                summary_parts.append("Data quality assessment shows good completeness with minor gaps.")
            else:
                summary_parts.append(f"Data quality issues identified with {total_missing} missing values.")
        ...
        if documentation_results.get('status') == 'completed':
            ...
            if completeness >= 0.9:
                summary_parts.append("Documentation review shows comprehensive compliance coverage.")
            elif completeness >= 0.7:
                summary_parts.append("Documentation review shows adequate compliance with minor gaps.")
            else:
                summary_parts.append("Documentation review identifies significant compliance gaps.")

        return " ".join(summary_parts)

    def _extract_key_metrics(self, validator_results: Dict[str, Any], 
                           audit_findings: Dict[str, Any],
                           compliance_assessment: Dict[str, Any]) -> Dict[str, str]:
        ...
        # Performance metrics
        if validator_results.get('status') == 'completed':
            validator_metrics = validator_results.get('metrics', {})
            metrics.update({
                'auc': f"{validator_metrics.get('auc', 0.0):.3f}",
                'ks_statistic': f"{validator_metrics.get('ks_statistic', 0.0):.3f}",
                'gini': f"{validator_metrics.get('gini', 0.0):.3f}",
                'psi': f"{validator_metrics.get('psi', 0.0):.3f}"
            })

        # Audit score
        independent_assessment = audit_findings.get('independent_assessment', {})
        audit_score = independent_assessment.get('overall_audit_score', 0.0)
        metrics['audit_score'] = f"{audit_score:.2f}"

        # Compliance status
        overall_compliance = compliance_assessment.get('overall_compliance', 'Unknown')
        metrics['compliance_status'] = overall_compliance

        # Approval status
        if audit_score >= 0.8:
            metrics['approval_status'] = 'Recommended'
        elif audit_score >= 0.6:
            metrics['approval_status'] = 'Conditional'
        else:
            metrics['approval_status'] = 'Not Recommended'

        return metrics

    def _generate_actionable_recommendations(self, audit_findings: Dict[str, Any],
        ...
        if audit_score >= 0.8:
            recommendations.extend([
                "Proceed with model implementation - all validation criteria met",
                "Establish quarterly performance monitoring schedule",
                "Document final validation results for regulatory reporting"
            ])
        elif audit_score >= 0.6:
            recommendations.extend([
                "Address identified issues before final approval",
                "Implement enhanced monitoring for model stability",
                "Schedule follow-up validation within 3 months"
            ])
        else:
            recommendations.extend([
                "Conduct comprehensive model review and improvement",
                "Address all critical validation gaps",
                "Consider alternative modeling approaches"
            ])

        # Issue-specific recommendations
        if critical_issues:
            recommendations.append("Priority actions required:")
        ...
        if compliance_issues:
            recommendations.append("Regulatory compliance actions:")
            for issue in compliance_issues[:2]:  # Top 2 compliance issues
                recommendations.append(f"• {issue}")

        return recommendations[:8]  # Limit to 8 recommendations for display
